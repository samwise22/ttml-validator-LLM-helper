...
async function upload(file){const selected=document.querySelector('input[name="presentation"]:checked');if(!selected){fail("Choose whether the subtitles are for horizontal or vertical presentation, then select the file again.");return}show("progress");$("#filename").textContent=file.name;$("#stage").textContent="Uploading TTML";const body=new FormData();body.append("file",file);body.append("presentation",selected.value);try{const r=await fetch("/api/jobs",{method:"POST",body});const data=await r.json();if(!r.ok)throw Error(data.detail||"Upload failed");poll(data.id)}catch(e){fail(e.message)}}
...
function finish(id,job){show("done");$("#doneTitle").textContent=job.sourceFilename;const order=["error","warning","info","pass"];$("#counts").innerHTML=order.map(k=>`<span class="${k}">${job.counts?.[k]||0} ${k}</span>`).join("");$("#view").href=`/api/jobs/${id}/report`;$("#view").target="_blank";$("#download").href=`/api/jobs/${id}/download/report`}
