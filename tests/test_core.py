...
class CoreTests(unittest.TestCase):
    def test_lossless_grouping_and_report(self):
        results = [
            {"status": 2, "code": "review_value", "message": "Value needs review", "location": "line 2"},
        ...
        self.assertEqual(ledger["occurrenceCount"], 4)
        self.assertEqual(ledger["findingGroups"][1]["occurrenceCount"], 2)
        ...
        self.assertIn("Not ready for delivery", output)
        self.assertIn("Errors — blocking", output)
        self.assertLess(output.index("Bad Value"), output.index("Review Value"))
